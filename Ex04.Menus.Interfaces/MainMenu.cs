...
namespace Ex04.Menus.Interfaces
{
    public class MainMenu
    {
        ...
        private readonly IConsoleMenuListener m_ConsoleMenuObserver;

        public MainMenu(IConsoleMenuListener i_ConsoleMenuObserver)
        {
            m_MenuItems = new List<MenuItem>();
            m_CurrItems = m_MenuItems;
            m_ConsoleMenuObserver = i_ConsoleMenuObserver;
        }
        ...
        {
            if (!i_BaseItem.Clickable)
            {
                throw new Exception("Unclickable item can't have sub items.");
            }
            else
            {
                foreach (MenuItem item in i_Items)
                {
                    i_BaseItem.addItem(item);
        ...
        private bool chooseInput()
        {
            ...
            {
                if (m_CurrItems[inputNumber - 1].Clickable)
                {
                    m_CurrItems = m_CurrItems[inputNumber - 1].NextLevel;
                }
                else
                {
                    notifyObservers(m_CurrItems[inputNumber - 1]);
                }
            }
            else
            {
        ...
        private void notifyObservers(MenuItem i_Item)
        {
            (m_ConsoleMenuObserver as IConsoleMenuListener).ActivateFunction(i_Item.FunctionToActivate);
